...
// Funktion zur Berechnung des Schadens zwischen zwei Pokémon
const calculateDamage = (attacker, defender) => {
  const baseDamage =
    (((2 * 50) / 5 + 2) * attacker.attack * 50) / defender.defense / 50 + 2;
  ...
  attacker.type.forEach((attackerType) => {
    defender.type.forEach((defenderType) => {
      if (
        ...
        typeEffectiveness[attackerType][defenderType]
      ) {
